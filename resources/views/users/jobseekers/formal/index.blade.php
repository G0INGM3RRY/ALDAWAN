...
@section('content')
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            @php
                ...
                $completed = 0;
                $total = 10; // Total sections for formal workers
            ...
            @if($percentage < 100)
                <div class="mt-3">
                    ...
                    <a href="{{ route('jobseekers.edit') }}" class="btn btn-success btn-sm">
    ...
    @if(Auth::user()->jobseekerProfile)
        @php $profile = Auth::user()->jobseekerProfile; @endphp
        @if($profile->hasEducationRecords())
            <div class="card mb-4">
                <div class="card-header bg-primary text-white">
                    <h5 class="mb-0"><i class="fas fa-graduation-cap me-2"></i>Education Background</h5>
                </div>
                <div class="card-body">
                    @foreach($profile->getEducationRecords() as $index => $education)
                        <div class="border rounded p-3 mb-3 {{ $loop->last ? '' : 'bg-light' }}">
                            <div class="row">
                                <div class="col-md-8">
                                    <h6 class="fw-bold text-primary mb-1">
                                        {{ $education['level'] ?? ($educationLevels->find($education['level_id'])->name ?? 'N/A') }}
                                    </h6>
                                    <p class="mb-1">
                                        <strong>Institution:</strong> {{ $education['institution_name'] ?? 'Not specified' }}
                                    </p>
                                    @if(!empty($education['degree_field']))
                                        <p class="mb-1">
                                            <strong>Degree/Field:</strong> {{ $education['degree_field'] }}
                                        </p>
                                    @endif
                                    @if(!empty($education['honors']))
                                        <p class="mb-1">
                                            <strong>Honors:</strong> <span class="badge bg-success">{{ $education['honors'] }}</span>
                                        </p>
                                    @endif
                                </div>
                                <div class="col-md-4 text-md-end">
                                    @if(!empty($education['graduation_year']))
                                        <span class="badge bg-secondary">{{ $education['graduation_year'] }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        @else
            <div class="card mb-4">
                ...
                <div class="card-body text-center text-muted py-4">
                    ...
                    <p class="mb-0">No education records added yet</p>
                </div>
            </div>
        @endif
    @endif
