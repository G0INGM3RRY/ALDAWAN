    ...
    <div class="row">
        <div class="col-md-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h1 class="h3 text-primary"><i class="fas fa-search me-2"></i>Browse Gigs & Jobs</h1>
                <p class="text-muted mb-0">{{ $jobs->count() }} gig{{ $jobs->count() !== 1 ? 's' : '' }} available</p>
            </div>
                ...
                <div class="card-body">
                    <form method="GET" action="{{ route('jobs.index') }}" id="jobSearchForm">
                        <div class="row g-3">
                            ...
                            <!-- Employment Type -->
                            <div class="col-md-4">
                                <label for="employment_type" class="form-label fw-semibold">Work Type</label>
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-select" id="sort" name="sort">
                                    ...
                                    <option value="salary_high" {{ request('sort') == 'salary_high' ? 'selected' : '' }}>Pay: High to Low</option>
                                    <option value="salary_low" {{ request('sort') == 'salary_low' ? 'selected' : '' }}>Pay: Low to High</option>
                                </select>
                            </div>

                            <!-- Salary Range -->
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Pay Range (PHP)</label>
                                <div class="row g-2">
                                    <div class="col-6">
            ...
            <div class="alert alert-primary alert-dismissible fade show d-flex align-items-center" role="alert">
                ...
                <div class="flex-grow-1">
                    ...
                    @if(request('search'))
                        <span class="badge bg-dark me-1">Search: {{ request('search') }}</span>
                    @endif
                    @if(request('location'))
                        <span class="badge bg-dark me-1">Location: {{ request('location') }}</span>
                    @endif
                    @if(request('classification') && isset($classifications))
                        <span class="badge bg-dark me-1">Category: {{ $classifications->find(request('classification'))->name ?? 'N/A' }}</span>
                    @endif
                    @if(request('employment_type'))
                        <span class="badge bg-dark me-1">Type: {{ ucfirst(str_replace('_', ' ', request('employment_type'))) }}</span>
                    @endif
                    @if(request('min_salary') || request('max_salary'))
                        <span class="badge bg-dark me-1">
                            Pay: PHP{{ request('min_salary') ? number_format(request('min_salary')) : '0' }} - 
                            PHP{{ request('max_salary') ? number_format(request('max_salary')) : '∞' }}
                        </span>
                    @endif
                </div>
                <a href="{{ route('jobs.index') }}" class="btn btn-sm btn-outline-dark">Clear All</a>
            </div>
            @endif
